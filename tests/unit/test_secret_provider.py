...
from roi_h.harness.secrets import LinuxSecretServiceStore, MacOSKeychainSecretStore
...
def test_linux_secret_service_write_does_not_put_value_in_process_arguments() -> None:
    secret = f"value-{uuid4().hex}"
    completed = CompletedProcess(args=[], returncode=0, stdout="", stderr="")

    with (
        patch("roi_h.harness.secrets.shutil.which", return_value="/usr/bin/secret-tool"),
        patch("roi_h.harness.secrets.subprocess.run", return_value=completed) as run,
    ):
        LinuxSecretServiceStore().set("project", "dev", "TOKEN", secret)
